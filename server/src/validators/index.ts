...
export const validateString: ValidatorFunction = (key: string, value: any) => {
  const errors = [];
  if (!value || typeof value !== "string") {
    errors.push(`wrong data format for key ${key}, must be string`);
  }
  return {
    errors,
    valid: !errors.length,
...
export const mergeValidate = (
  ...
  for (const validator of validators) {
    const { valid: innerValidatorValid, errors: innerValidatorErrors } =
      validator(key, value);
