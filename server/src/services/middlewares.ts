...
export const errorHandler = (
  error: HttpException | MongooseError,
  req: Request,
  res: Response,
  next: NextFunction
) => {
  const response: IAPIError = {
    success: false,
    status: 500,
    errors: [{ code: APIErrorType.API_INTERNAL_ERROR }],
  ...
  if (error.name == "HttpException") {
    ...
    response.errors = errorCodes.map(
      (err: MongooseError) => <IErrorCode>{ code: err.message }
    );
  } else {
...
export const bearerTokenHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  return function (req: Request, res: Response, next: NextFunction) {
    if (res.locals.decoded) {
      ...
      return next(
        new HttpException(
          401,
          APIErrorType.API_UNAUTHORIZED,
          "You do not have rights to perform this action, if you think it is a mistake please contact your administrator"
        )
      );
    }
    return next(
...
export function authorizeQueryParams(
  authorizedParametersObject: { [key: string]: boolean } = {}
) {
  return function (req: Request, res: Response, next: NextFunction) {
    const { query } = req;
    if (query) {
...
export function authorizeBodyParams(
  authorizedParametersObject: { [key: string]: boolean } = {}
) {
  return function (req: Request, res: Response, next: NextFunction) {
    const { body } = req;
...
export function requireBodyParams(
  requiredParametersObject: { [key: string]: boolean } = {}
) {
  return function (req: Request, res: Response, next: NextFunction) {
    const { body } = req;
    if (body) {
